...
const NavigationItem: FC<NavigationItemProps> = ({ menuItem }) => {
        ...
        {() => (
          <>
            ...
            >
              <Popover.Panel
                static
                className="sub-menu absolute transform z-10 w-56 top-full left-0 bg-white dark:bg-neutral-900 shadow-xl rounded-lg"
              >
                <ul className="rounded-lg text-sm relative py-4 grid space-y-1">
                  {menuDropdown.children?.map((i) => {
                    if (i.type) {
                      return renderDropdownMenuNavlinkHasChild(i);
                    } else {
        ...
        {() => (
          <>
            ...
            >
              <Popover.Panel
                static
                className="sub-menu absolute z-10 w-56 left-full pl-2 top-0 bg-white dark:bg-neutral-900 shadow-xl rounded-lg"
              >
                <ul className="rounded-lg text-sm relative py-4 grid space-y-1">
                  {item.children?.map((i) => {
                    if (i.type) {
  ...
  const renderMainItem = (item: NavItemType) => {
    return (
      <div className="h-20 flex-shrink-0 flex items-center">
        <Link
          className="inline-flex items-center text-sm lg:text-[15px] font-medium text-slate-700 dark:text-slate-300 py-2.5 px-4 xl:px-5 rounded-full hover:text-slate-900 hover:bg-slate-100 dark:hover:bg-slate-800 dark:hover:text-slate-200 transition-all ease-in-out duration-300"
          href={{
            pathname: item.href || undefined,
          }}
        >
